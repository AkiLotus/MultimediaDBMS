...
def distance(data1: Data, data2: Data, metric: int = 2) -> int:
	assert (data1.dimension == data2.dimension)
...
def raw_distance(data1: List, data2: List, metric: int = 2) -> int:
...
def distance_to_slice(data: Data, cut_layer: tuple, metric: int = 2) -> int:
...
class KDTree:
	def __init__(self, data_list: List[List]):
	...
	def search(self, data: Data, tolerance: float = 0.0):
